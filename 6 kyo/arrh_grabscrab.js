/*
DESCRIPTION:
Pirates have notorious difficulty with enunciating. 
They tend to blur all the letters together and scream at people.

At long last, we need a way to unscramble what these pirates are saying.

Write a function that will accept a jumble of letters as well as a dictionary, 
and output a list of words that the pirate might have meant.

For example:

grabscrab( "ortsp", ["sport", "parrot", "ports", "matey"] )
Should return ["sport", "ports"].

Return matches in the same order as in the dictionary. Return an empty array if there are no matches.

Good luck!


ПЕРЕВОД:
Пираты испытывают огромные трудности с произношением слов. 
Они обычно смешивают все буквы вместе и кричат на людей.

Наконец-то нам нужен способ расшифровать то, что говорят пираты.

Напишите функцию, которая будет принимать набор букв, а также словарь, 
и выводить список слов, которые пират мог иметь в виду.

Например:

grabscrab("ortsp", ["sport", "parrot", "ports", "matey"] )
Должно вернуться ["спорт", "порты"].

Возвращать совпадения в том же порядке, что и в словаре. Вернуть пустой массив, если совпадений нет.
*/